...
3. Configure at least one authentication provider (Email, Google, GitHub, Facebook or GitLab). More info here: [Configuration](''/self-hosting/configuration).
...
services:
  ...
  typebot-builder:
    ...
    # See ''/self-hosting/configuration for more configuration options
    env_file:
      - .env
  typebot-viewer:
    labels:
