...
test('folders navigation should work', async ({ page }) => {
  ...
  await page.fill('input[value="Nova Pasta"]', 'Minha Pasta #1')
  await Promise.all([
    page.waitForResponse((resp) => resp.request().method() === 'PATCH'),
    page.press('input[value="Minha Pasta #1"]', 'Enter'),
  ])
  await page.click('li:has-text("Minha Pasta #1")')
  await expect(page.locator('h1:has-text("Minha Pasta #1")')).toBeVisible()
  await createFolderButton.click()
  await page.click('text="Nova Pasta"')
  await page.fill('input', 'Minha Pasta #2')
  await page.press('input', 'Enter')

  await page.click('li:has-text("Minha Pasta #2")')
  await expect(page.locator('h1 >> text="Minha Pasta #2"')).toBeVisible()

  await page.click('text="Back"')
  await expect(page.locator('span >> text="Minha Pasta #2"')).toBeVisible()

  await page.click('text="Back"')
  await expect(page.locator('span >> text=Minha Pasta #1')).toBeVisible()
})
